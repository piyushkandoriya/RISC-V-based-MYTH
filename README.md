...
```
Code and log file from mackerchip is given below,

<img width="960" alt="image" src="https://github.com/piyushkandoriya/RISC-V-based-MYTH/assets/123488595/dd9ac995-f47c-4e54-a0f1-ac06c1bfa3c9">

Here also we are getting "PASSED" signal which we can see in above image.

### Lab to modify 3-Cycle RISC-V to distribute logic
Link for modified 3-cycle RISC_v with distributed logic is : ```https://myth.makerchip.com/sandbox/0rkfAhzM5/01jhym#```
...
\TLV
   ...
   |cpu
      ...
      @3
         ...
         $valid_taken_br = $valid && $taken_br; //To invalid the branch taken during invalid instruction or invalid clock
      ...
      // Register File Write
         $rf_wr_en = ($rd_valid && $valid && $rd != 5'b0) || >>2$valid_load; //to enable write when load instruction was there before 2 cycle
         ?$rf_wr_en
            $rf_wr_index[4:0] = !$valid ? >>2$rd[4:0] : $rd[4:0];    //to enable write the value in RF when load instruction was there before 2 cycle otherwise it will write the value from rd register only
      
         $rf_wr_data[31:0] = !$valid ? >>2$ld_data[31:0] : $result[31:0];   // $result is coming from ALU and getting stored in $rf_wr_index[4:0] address of RISC-V register        
      ...
      @4
         $dmem_rd_en = $valid_load;  //connecting the interface of data memory
         $dmem_wr_en =$is_s_instr && $valid ; //connecting the interface of data memory
         $dmem_addr[3:0] = $result[5:2];
